...
<html>
	...
	<form id="mainForm" action="org.action?lookup=${lookup }" method="get">
		...
		<table align="center" border="0" cellpadding="0"
			...
			<tr>
				...
				<c:choose>
					...
					<input type='button' onclick="addNew('org!input.action')" class='button_70px' value='新建'/>
					</c:when>
					<c:otherwise>
		...
		<table class="table_all" align="center" border="0" cellpadding="0"
			...
			<s:iterator value="page.result">
				<tr>
				    ...
				    <c:choose>
				    <c:when test="${empty lookup}">
						<a href="org!delete.action?id=${id }" class="btnDel" title="删除" onclick="return confirmDel();">删除</a>
						<a href="org!input.action?id=${id }" class="btnEdit" title="编辑">编辑</a>
						<a href="org!input.action?id=${id }" class="btnView" title="查看">查看</a>
					</c:when>
					<c:otherwise>
			...
			<frame:page exportUrl="org!export.action" curPage="${page.pageNo}" totalPages="${page.totalPages }" totalRecords="${page.totalCount }" lookup="${lookup }"/>
		</table>
	</form>
	</body>
</html>
